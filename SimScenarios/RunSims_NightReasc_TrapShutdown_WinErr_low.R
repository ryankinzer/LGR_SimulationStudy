...
while(i <= n_sim) {
  ...
  lgr_week = SimulateLGRobs(my_sim$parameters, lgr_truth, error_rate = 0.05, perfect.window = F)
...
# save results
save(res, file = 'SimulationFits/Sim_NightReasc_TrapShutdown_WinErr_low.rda')
# save(sim_list, obs_list, mod_list, file = 'SimulationFits/Sim_NightReasc_TrapShutdown_WinErr_low_other.rda')
